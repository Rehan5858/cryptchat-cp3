...
import static com.example.cryptchat_cp.EncryptionDecryption.AES.decrypt;
...
public class BlowFish {
    ...
    public BlowFish(String msgId){
        SECRET_KEY = msgId;
    }
    private static final String SALT = "1427@1427@1427@1427$$%%@#*&^";
    String msgId, key = "aabb09182736ccdd", cipherText;
    @RequiresApi(api = Build.VERSION_CODES.O)
    public String encryptBlowFish(String msgId){
       this.msgId = msgId;
        for (int i = 0; i < 32; i++)
            modVal = modVal << 1;

        keyGenerate(key);
       return  encrypt2(msgId);
    }
    public String decryptBlowFish(String cipherText){
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    public String encrypt2(String msgId){
